...
/* ---------------------------------------------------------------------- */
	...
	$.include('frontend/js/jquery.cycle.all.min.js');
	$.include('frontend/js/respond.min.js');
	
	if(jQuery('.video-image').length) {
		$.include('frontend/fancybox/jquery.fancybox.pack.js');
	}
	
	//	Panel
	$.include('frontend/changer/js/changer.js');
	$.include('frontend/changer/js/colorpicker.js');
...
jQuery(document).ready(function($) {
	...
	(function() {
		...
		if($('#contactform').length) {
			...
			$loader = '<img src="frontend/images/preloader.gif" alt="Loading..." />';
			...
			$form.submit(function(e){
				...
				//send data to server
				$.post("frontend/php/contact-send.html", data, function(response) {
			...
			if($('a.video-image').length) {
				(function() {
					$('a.video-image').on('click',function() {
						$.fancybox({
							'type' : 'iframe',
							'href' : this.href.replace(new RegExp('watch\\?v=', 'i'), 'frontend/embed/index.html') + '&autoplay=1',
							'overlayShow' : true,
							'centerOnScroll' : true,
							'speedIn' : 100,
							'speedOut' : 50,
