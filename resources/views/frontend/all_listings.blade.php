		...
		<section class="container sbl clearfix">
			...
			<section id="content" class="two-thirds column">
				...
				<section id="change-items" class="item-grid">
					<?php 
						$record_per_page = 9;
						$product = DB::table("tbl_product")->orderBy("pk_product_id","desc")->paginate($record_per_page);
					 ?>
			...
			<aside id="sidebar" class="one-third column">
				...
				<div class="widget-container widget_latest">
					...
					<?php 
						$latestproduct = DB::table("tbl_product")->orderBy("pk_product_id","desc")->paginate(3);
					 ?>
					<ul>
						@foreach($latestproduct as $rows)
						<li>
							<a class="thumb single-image picture video" href="{{ url('product/'.$rows->pk_product_id) }}"><img src="{{ asset('upload/product/'.$rows->c_img) }}" alt="" /></a>
							<div class="latest-entry">
								<a href="{{ url('product/'.$rows->pk_product_id) }}" class="block"><b>{{ $rows->c_name }}</b></a>
								<span class="block add-margin">2009 y.r., 160,000 km.</span>
								<a href="{{ url('product/'.$rows->pk_product_id) }}" class="bold text-orange">${{ number_format($rows->c_price) }}</a>	
							</div><!--/ .latest-entry-->
						</li>
						@endforeach
					</ul>
